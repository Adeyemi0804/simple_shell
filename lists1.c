#include "kade.h"

/**
 * kade_list_len - determines the length of a linked list
 * @h: pointer to the first node
 *
 * Return: size of the list
 */
size_t kade_list_len(const list_t *h)
{
    size_t i = 0;

    for (; h; h = h->next)
        i++;

    return (i);
}

/**
 * kade_list_to_strings - returns an array of strings of the list->str
 * @head: pointer to the first node
 *
 * Return: array of strings
 */
char **kade_list_to_strings(list_t *head)
{
    list_t *node = head;
    size_t i = kade_list_len(head), j;
    char **strs;
    char *str;

    if (!head || !i)
        return (NULL);

    strs = malloc(sizeof(char *) * (i + 1));
    if (!strs)
        return (NULL);

    for (i = 0; node; node = node->next, i++)
    {
        str = malloc(kade_strlen(node->str) + 1);
        if (!str)
        {
            for (j = 0; j < i; j++)
                free(strs[j]);
            free(strs);
            return (NULL);
        }

        str = kade_strcpy(str, node->str);
        strs[i] = str;
    }

    strs[i] = NULL;
    return (strs);
}

/**
 * kade_print_list - prints all elements of a list_t linked list
 * @h: pointer to the first node
 *
 * Return: size of list
 */
size_t kade_print_list(const list_t *h)
{
    size_t i = 0;

    for (; h; h = h->next)
    {
        kade_puts(kade_convert_number(h->num, 10, 0));
        kade_putchar(':');
        kade_putchar(' ');
        kade_puts(h->str ? h->str : "(nil)");
        kade_puts("\n");
        i++;
    }

    return (i);
}

/**
 * kade_node_starts_with - returns a node whose string starts with prefix
 * @node: pointer to list head
 * @prefix: string to match
 * @c: the next character after the prefix to match
 *
 * Return: matching node or null
 */
list_t *kade_node_starts_with(list_t *node, char *prefix, char c)
{
    char *p = NULL;

    while (node)
    {
        p = kade_starts_with(node->str, prefix);
        if (p && ((c == -1) || (*p == c)))
            return (node);
        node = node->next;
    }

    return (NULL);
}

/**
 * kade_get_node_index - gets the index of a node
 * @head: pointer to list head
 * @node: pointer to the node
 *
 * Return: index of node or -1
 */
ssize_t kade_get_node_index(list_t *head, list_t *node)
{
    size_t i = 0;

    while (head)
    {
        if (head == node)
            return (i);
        head = head->next;
        i++;
    }

    return (-1);
}
